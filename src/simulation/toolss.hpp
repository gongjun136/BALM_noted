...
public:
  ...
  Eigen::Matrix<double, 6, 3> BP_cov;
  Eigen::Matrix<double, 9, 9> c_cov;
  #endif
  ...
  void push(const Eigen::Vector3d &vec)
  {
    ...
    double pn = pnoise;
    Eigen::Matrix3d p_cov = I33 * pn * pn;

    P_cov += Bi * p_cov * Bi.transpose();
    BP_cov += Bi * p_cov;
    v_cov += p_cov;

    Eigen::Matrix<double, 9, 3> Bf;
    Bf.block<6, 3>(0, 0) = Bi;
    Bf.block<3, 3>(6, 0).setIdentity();
    c_cov += Bf * p_cov * Bf.transpose();
